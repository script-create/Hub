...
do
    ...
    do
        ...
        local function visualCharacter(player)
            return player and player.Character
        end

        local function visualRoot(player)
            local character = visualCharacter(player)
        ...
        local function visualHumanoid(player)
            local character = visualCharacter(player)
            return character and character:FindFirstChildOfClass('Humanoid')
        end

        local function removeVisualLabel(player)
        ...
        local function removeVisualHighlight(player)
        ...
        local function removeVisualTracer(player)
        ...
        local function clearVisualESP()
            for player in pairs(VisualHighlights) do removeVisualHighlight(player) end
            for player in pairs(VisualLabels) do removeVisualLabel(player) end
            for player in pairs(VisualTracers) do removeVisualTracer(player) end
        end

        local function addVisualHighlight(player)
        ...
        local function addVisualLabel(player)
            if player == VisualLocalPlayer then return end
            local character = visualCharacter(player)
            local root = visualRoot(player)
            if not character or not root then return end

            removeVisualLabel(player)
        ...
        local function updateVisualLabels()
            for player, billboard in pairs(VisualLabels) do
                local character = visualCharacter(player)
                local root = visualRoot(player)
                local humanoid = visualHumanoid(player)
        ...
        local function updateDrawingTracers()
            if not Drawing then return end
        ...
        local function refreshVisualESP()
            for _, player in ipairs(VisualPlayers:GetPlayers()) do
                if player ~= VisualLocalPlayer then
                    addVisualHighlight(player)
                    addVisualLabel(player)
                    createDrawingTracer(player)
                end
            end
        end

        local function removeAllVisualConnections()
        ...
        local function setVisualESP(enabled)
            removeAllVisualConnections()
            clearVisualESP()
            ...
            table.insert(VisualConnections, VisualPlayers.PlayerRemoving:Connect(function(player)
                removeVisualHighlight(player)
                removeVisualLabel(player)
                removeVisualTracer(player)
            end))
        end

        local function setSelfChams(enabled)
        ...
        table.insert(VisualConnections, VisualRunService.RenderStepped:Connect(function()
            if next(VisualLabels) then updateVisualLabels() end
            if next(VisualTracers) then updateDrawingTracers() end
        end))

        VisualPlayers.PlayerAdded:Connect(function(player)
            player.CharacterAdded:Connect(function()
                task.wait(0.5)
                if VisualSelfChams and player == VisualLocalPlayer then setSelfChams(true) end
            end)
        end)
        ...
        -- Skybox controls moved into Visuals.
        local VisualSkyPresets = {
            {Name = 'Red', Id = '98490421374360'},
            {Name = 'Pink', Id = '95000769820905'},
            {Name = 'Pink 2', Id = '82988835868087'},
            {Name = 'Green', Id = '5036205687'},
            {Name = 'Black', Id = '80807192441609'},
            {Name = 'Cosmic', Id = '77816282467771'},
            {Name = 'Yellow', Id = '2669948520'},
        }
        local VisualSkyOriginal = nil
        local VisualCustomSky = false

        do
            local sky = VisualLighting:FindFirstChildOfClass('Sky')
            if sky then
                VisualSkyOriginal = {
        ...
        local function restoreVisualSky()
            for _, child in ipairs(VisualLighting:GetChildren()) do
                if child:IsA('Sky') or child:IsA('Atmosphere') or child:IsA('Clouds') then
                    child:Destroy()
                end
            end
            if VisualSkyOriginal then
                local sky = Instance.new('Sky')
                for key, value in pairs(VisualSkyOriginal) do sky[key] = value end
                sky.Parent = VisualLighting
            end
            VisualCustomSky = false
        end

        local function applyVisualSky(id)
            for _, child in ipairs(VisualLighting:GetChildren()) do
                if child:IsA('Sky') or child:IsA('Atmosphere') or child:IsA('Clouds') then
                    child:Destroy()
                end
            ...
            sky.SunAngularSize, sky.StarCount = 0, 0
            sky.Parent = VisualLighting
            VisualLighting.ClockTime = 14
            VisualLighting.Brightness = 2
            VisualLighting.GlobalShadows = false
            VisualLighting.FogEnd = 999999
            VisualCustomSky = true
        end

        VisualsTab:Divider()
        VisualsTab:Paragraph({
            Title = 'Skybox',
            Content = 'Skybox presets and restore-to-default control from the archive.',
        })
        local VisualSkyNames = {}
        for _, preset in ipairs(VisualSkyPresets) do table.insert(VisualSkyNames, preset.Name) end
        VisualsTab:Dropdown({
            Title = 'Skybox Preset',
            Values = VisualSkyNames,
            Value = VisualSkyNames[1],
            Callback = function(value)
                for _, preset in ipairs(VisualSkyPresets) do
                    if preset.Name == value then
                        applyVisualSky(preset.Id)
                        break
                    end
                end
            end,
        ...
        VisualsTab:Button({
            Title = 'Restore Default Skybox',
            Description = 'Remove the custom sky and restore the original skybox.',
            Callback = function()
                restoreVisualSky()
            end,
        })

        -- Crosshair controls moved into Visuals.
        local VisualCrosshairPresets = {
        ...
        local VisualCrosshairGui = nil
        local VisualCrosshairImage = nil
        local VisualCrosshairSize = 44
        local VisualCrosshairRotation = false
        local VisualCrosshairSpinConnection = nil

        local function destroyVisualCrosshair()
            if VisualCrosshairSpinConnection then
                VisualCrosshairSpinConnection:Disconnect()
                VisualCrosshairSpinConnection = nil
            end
            if VisualCrosshairGui then
                VisualCrosshairGui:Destroy()
                VisualCrosshairGui = nil
                VisualCrosshairImage = nil
            end
        end

        local function createVisualCrosshair(id)
            destroyVisualCrosshair()
            local gui = Instance.new('ScreenGui')
            gui.Name = 'CrystalHub_VisualCrosshair'
            ...
            image.Size = UDim2.fromOffset(VisualCrosshairSize, VisualCrosshairSize)
            image.BackgroundTransparency = 1
            image.Image = 'rbxassetid://' .. tostring(id)
            image.Parent = gui
            VisualCrosshairGui = gui
            VisualCrosshairImage = image

            if VisualCrosshairRotation then
                VisualCrosshairSpinConnection = VisualRunService.RenderStepped:Connect(function()
                    if VisualCrosshairImage and VisualCrosshairImage.Parent then
                        VisualCrosshairImage.Rotation = (VisualCrosshairImage.Rotation + 4) % 360
                    end
                end)
            end
        end

        local VisualCrosshairNames = {}
        for _, preset in ipairs(VisualCrosshairPresets) do table.insert(VisualCrosshairNames, preset.Name) end
        VisualsTab:Divider()
        VisualsTab:Paragraph({
            Title = 'Crosshair',
            Content = 'Custom crosshair presets, size and optional rotation.',
        })
        VisualsTab:Toggle({
            ...
            Callback = function(value)
                if value then
                    createVisualCrosshair(VisualCrosshairPresets[1].Id)
                else
                    destroyVisualCrosshair()
                end
            end,
        ...
        VisualsTab:Dropdown({
            ...
            Values = VisualCrosshairNames,
            Value = VisualCrosshairNames[1],
            Callback = function(value)
                for _, preset in ipairs(VisualCrosshairPresets) do
                    if preset.Name == value and VisualCrosshairGui then
                        createVisualCrosshair(preset.Id)
                        break
                    end
                end
            end,
        ...
        VisualsTab:Slider({
            ...
            IsTooltip = true,
            IsTextbox = true,
            Value = {Min = 12, Max = 100, Default = VisualCrosshairSize},
            Callback = function(value)
                VisualCrosshairSize = tonumber(value) or 44
                if VisualCrosshairImage then
                    VisualCrosshairImage.Size = UDim2.fromOffset(VisualCrosshairSize, VisualCrosshairSize)
                end
            end,
        })
        VisualsTab:Toggle({
            ...
            Default = false,
            Callback = function(value)
                VisualCrosshairRotation = value
                if VisualCrosshairImage and value then
                    if VisualCrosshairSpinConnection then VisualCrosshairSpinConnection:Disconnect() end
                    VisualCrosshairSpinConnection = VisualRunService.RenderStepped:Connect(function()
                        if VisualCrosshairImage and VisualCrosshairImage.Parent then
                            VisualCrosshairImage.Rotation = (VisualCrosshairImage.Rotation + 4) % 360
                        end
                    end)
                elseif VisualCrosshairSpinConnection then
                    VisualCrosshairSpinConnection:Disconnect()
                    VisualCrosshairSpinConnection = nil
                    if VisualCrosshairImage then VisualCrosshairImage.Rotation = 0 end
                end
            end,
        })

        -- Low Graphics / Graphics Settings moved into Visuals.
        local VisualLowGraphics = false
        local VisualGraphicsOriginal = {
            GlobalShadows = VisualLighting.GlobalShadows,
            Brightness = VisualLighting.Brightness,
            Ambient = VisualLighting.Ambient,
            OutdoorAmbient = VisualLighting.OutdoorAmbient,
        }
        local VisualPartOriginal = {}

        local function applyVisualLowGraphics(instance)
            if instance:IsA('BasePart') then
                if not VisualPartOriginal[instance] then
                    VisualPartOriginal[instance] = {
            ...
            elseif instance:IsA('Decal') or instance:IsA('Texture') then
                if not VisualPartOriginal[instance] then
                    VisualPartOriginal[instance] = {Transparency = instance.Transparency}
                end
                instance.Transparency = 1
            end
        end

        local function setVisualLowGraphics(enabled)
            VisualLowGraphics = enabled
            if enabled then
                ...
                for _, instance in ipairs(workspace:GetDescendants()) do
                    applyVisualLowGraphics(instance)
                end
            else
                pcall(function() settings().Rendering.QualityLevel = Enum.QualityLevel.Automatic end)
                VisualLighting.GlobalShadows = VisualGraphicsOriginal.GlobalShadows
                VisualLighting.Brightness = VisualGraphicsOriginal.Brightness
                VisualLighting.Ambient = VisualGraphicsOriginal.Ambient
                VisualLighting.OutdoorAmbient = VisualGraphicsOriginal.OutdoorAmbient
                for instance, props in pairs(VisualPartOriginal) do
                ...
                table.clear(VisualPartOriginal)
            end
        end
        ...
        VisualsTab:Paragraph({
            ...
            Content = 'Low Graphics mode from the archive: lowers rendering quality, removes part shadows and hides decals/textures.',
        })
        VisualsTab:Toggle({
            Title = 'Low Graphics',
            Default = false,
            Callback = function(value)
                setVisualLowGraphics(value)
            end,
        })
        VisualsTab:Button({
            Title = 'Restore Graphics',
            Description = 'Restore the saved lighting and part/texture properties.',
            Callback = function()
                setVisualLowGraphics(false)
            end,
        })
    end
